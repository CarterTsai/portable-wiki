...
*/
static void malformed_request(int code, char *info)
{
 ...
 */
static void http_request_parse_params(HttpRequest *req,
            char *data,
            int terminator)
{
	while (*data) {
...
char *http_request_checkbox_get(HttpRequest *req)
{
	int i, lenght = 0;
	static char string[1000]; /* FIXME: limitation */
...
char *ahttp_request_get_uri(HttpRequest *req)
{
	return req->uri;
}
...
void http_response_set_status(HttpResponse *res,
    int status_code,
    char *status_desc)
{
...
/* ! file loaded in mem */
void http_response_send_smallfile(HttpResponse *res, char *filename,
        char *content, unsigned long sizelimit)
{
	unsigned char *data;
...
void http_response_send_bigfile(HttpResponse *res, char *filename,
      char *content)
{
	http_response_set_content_type(res, content);
	http_response_send_headers(res);
...
void sigint(int sig)
{
	printf("\nPortableWiki stopped.\n");
	syslog(LOG_LOCAL0 | LOG_INFO, "PortableWiki stopped.\n");
...
HttpRequest *http_server(struct in_addr address, int iPort)
{
	...
	fprintf(stderr, "PortableWiki firing up ...\n");
	...
	fprintf(stderr, "PortableWiki Started. Please point your browser at %s:%i\n", inet_ntoa(address), iPort);

	/* log starting information */
	openlog("portablewiki", 0, 0);
	syslog(LOG_LOCAL0 | LOG_INFO, "started with PID %d", getpid());

	/* Set PORTABLEWIKI_URL_PREFIX if not already set - rss uses it */
	snprintf(url_prefix, 256, "%s:%i/", inet_ntoa(address), iPort + n);
	setenv("PORTABLEWIKI_URL_PREFIX", url_prefix , 0);
