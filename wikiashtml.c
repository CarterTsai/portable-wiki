...
static int pic_width = 0, pic_height = 0, pic_border = 0;
static int expand_collapse_num = 0;
static int target_wwwlink = 0;
...
static char *check_for_link(char *line, int *skip_chars)
{
	...
	if (*p == '[') {
		/* FIXME: Allow links like [the Main page] ( covert to the_main_page )
		 */
		url = start + 1;
		*p = '\0';
		p++;
	...
	/* youtube video embedded */
		...
		*start = '\0';
		lgasprintf = asprintf(&result,
			"<object width=\"480\" height=\"385\"><param name=\"movie\" value=\"%s\"></param>"
...
void wiki_parse_between_braces(char *s)
{
	char *str_ptr;
...
void search_header(char **sectionlist, char *raw_page_data)
{
	int header = 0;
	int i = 0;
